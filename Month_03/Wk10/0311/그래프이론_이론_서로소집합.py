...
'''
현재 find 함수가 비효율 적으로 동작중
만약 4개의 union 연산이 (4,5)(3,4)(2,3)(1,2) 와 같이 주어진다면
모든 원소가 루트노드로 1을 가지며 최대 O(v)의 시간이 소요된다.
결과적으로 노드가 V개 find 혹은 union연산이 M개 일떄 O(VM)이 되므로 비효율적이다.

경로 압축 기법을 통해 시간복잡도를 개선가능
    - 경로 압축은 find 함수를 재귀적으로 호출 한 뒤에 부모 테이블 값을 갱신하는 기법
    - 기존 find 함수를 아래와 같이 변경하면 구현 완료
'''
...
# 각 원소가 속한 집합 출력
for i in range(1, v+1):
    print(find_parent_2(parent,i), end='')
print()
