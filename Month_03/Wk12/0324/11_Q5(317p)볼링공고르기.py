...
# 볼링공의 갯수 N, 공의 최대무게 M 입력
n, m = map(int, input().split())

# 공의 종류 입력
data = list(map(int, input().split()))
# 공 무게마다 갯수를 저장할 리스트 입력, 공의 무게는 10까지 제한되어 있기 때문에 11개의 공간 생성
arr = [0] * 11

for i in data:
    arr[i] += 1 # 해당 무게의 공의 갯수 만큼 더하기 1

result = 0 # 최종 정답을 입력하기 위한 변수

for i in range(1, m+1): # 공의 무게의 갯수만큼 반복
    n -= arr[i] # 갯수 - 해당 무게의 공의 갯수(A가 선택할 수 있는 경우의 수) n에서 빼주고 다시 초기화 하지 않는 이유 = 중복방지
    result += arr[i] * n # 남은공의갯수(B가 선택할 공의 갯수) * (A가 선택할 경우의수)
