...
def solution(s):
    ...
    for unit in range(1, (len(s) //2 )+1):
        previous = s[0:unit] # 실제 슬라이싱하여 비교할 변수
        result = "" # 결과값 저장
        count = 1 # count
...
def solution(s):
    min_len = len(s) # 최초 문자열의 길이를 저장
    
    #탐색 범위는 1글자부터 문자열의 중간까지의 길이까지(절반)
    for unit in range(1, (len(s)//2)+1):
        result = "" # 결과 문자열
        
        previous = s[0:unit] # 첫 문자 탐색 범위이자 이전 탐색범위
        
        count = 1 # 탐색 문자 갯수

        # 탐색 인덱스를 탐색 범위만큼 증가시키면서, 이전 탐색범위와 비교
        for idx in range(unit, len(s), unit):
            now = s[idx:idx+unit] # 현재 탐색단위로 슬라이싱
            ...
            # 현재 탐색 단위가 이전 탐색 단위와 다르다면
                ...
                if count >= 2:
                    result += str(count) + previous # result에 결과값 입력
                else:
                    result += previous # result에 결과값 입력
                previous = now # 이전 탐색범위 최신화
                count = 1 # count 초기화
        # 남아있는 문자열 처리
        if count >= 2:
            result += str(count) + previous
            ...
            result += previous
        # 탐색 단위별로 압축 문자열의 길이를 비교해, 최소 압축 문자열 길이를 저장
        min_len = min(min_len, len(result))
    return min_len
