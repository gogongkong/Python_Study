...
def solution(s):
    ...
    for unit in range(1, ((len(s)//2)+1)):
        result = ""
        count = 1
        previous = s[0:unit] # 탐색할 문자열

        for i in range(1, len(s), unit):
            now = s[i: i+unit]
            if now == previous:
                count +=1
            else:
...
# def solution(s):
#     min_len = len(s) # 최초 문자열의 길이 저장

#     # 탐색 문자의 범위 지정 1글자부터 중간길이까지
#     for unit in range(1, (len(s)//2)+1):
#         previous = s[0:unit] # 탐색할 문자 범위, 처음 탐색문자
#         result = "" # 결과를 저장할 문자열
#         count = 1

#         for idx in range(1, len(s), unit):
#             now = s[idx:idx+unit] # 탐색범위만큼 슬라이싱
...
#         else:
#             result += previous
#         #탐색 단위별로 압출 문자열의 길이를 비교해 최소 압축문자열을 저장
#         min_len = min(min_len, len(result))
#     return min_len
