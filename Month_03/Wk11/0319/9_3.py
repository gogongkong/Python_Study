...
def DS(start):
    q = []
    heapq.heappush(q,(0, start)) # q,(비용, 시작점(노드))
    distance[start] = 0 # 시작점 비용은 0처리
    while q: # 큐가 빌 때까지
        dist, now = heapq.heappop(q) # dist와 now 우선순위 큐에서 뽑아냄
        if dist > distance[now]: # 이전에 최소 경로를 넣은 적이 있다면 continue
            continue
        for arrs in arr[now]:
            cost = dist + arrs[1]
