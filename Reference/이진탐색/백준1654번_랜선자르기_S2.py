...
k, n = map(int,input().split()) # 랜선의 갯수 K, 필요한 랜선의 갯수 N

data = [int(input()) for _ in range(k)] # 랜선의 정보를 입력받아 저장되어 있는 리스트 data

start, end = 1, max(data)

while start <= end:
    mid = (start + end) // 2
    result = 0 # 잘린 랜선의 갯수를 담을 변수
    for lan in data:
        result += lan // mid
    if result >= n: # 잘린 랜선의 갯수가 원하는 것보다 많으므로 더 적게 자른다
        start = mid + 1
    else: # 잘린 랜선의 갯수가 원하는 것보다 적으므로 더 많이 자른다.
        end = mid - 1
