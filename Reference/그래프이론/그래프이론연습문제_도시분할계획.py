...
# 풀이방향
# 하나의 최소 신장트리를 만들고 가장 큰 간선을 제거하면 2개의 최소 신장트리를 만들 수 있다.
...
# n : 집(노드)의 갯수, m : 길(간선의 갯수)
n, m = map(int, input().split())

data = [] # 길의 정보를 담을 리스트
for _ in range(m):
    a, b, cost = map(int, input().split()) # 길의 정보 입력
    data.append((cost, a, b)) # 비용별로 정렬 하기 위해 cost 부터 입력
data.sort()
